...
- [ADR-012 — Flarex Runtime Kernel & Evaluation Engine Specification](012-flarex-runtime-kernel.md) — **Accepted** (governing spec; amends ADR-007 §soft-degrade, §dirty-key, §view-dot) — **implementation complete 2026-08-04**: 33/33 slices, all 11 rollout flags removed, one execution path. Report: `plans/adr-012-completion-report.md`. The ADR stays normative for future runtime work; completion is a property of the programme, not of the decision.
...
- [ADR-018 — Evidence Corrections](018-evidence-corrections.md) — **Accepted** (A1–A5, B1–B5, C1; ADR-016's admission test applied to the ADRs that predate it, while the corpus is still empty — amends E3/E11/E13, D8, I9/I12, U1/U4 and extends ADR-016 by one clause, I15. A5 gates every new write on single-writer discipline. Amends clauses only: ADR-014, 015, 016 and 017 all stay Accepted and normative)
- [ADR-022 — Source colour: input transforms, the working space, and the verification gate](022-source-colour-input-transforms.md) — **Accepted** (§2–§5 normative; §6 Provisional). A transfer function represents the ENCODING, not editor policy, so diffuse-white normalisation is a separate named step HDR export can decline. Four-level verification ladder with `spec-checked` reserved for HUMAN review — agent comparison caps at `corroborated`, and only `spec-checked` clears a format for renderer wiring. Published specs are reproduced faithfully INCLUDING their defects (F-Log's toe discontinuity is Fujifilm's). Working space stated at last: linear Rec.709 D65 — Provisional, because it clips wide-gamut log before the grade.
