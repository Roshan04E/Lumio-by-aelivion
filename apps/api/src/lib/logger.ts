...
/**
 * Kimera AI — tiny debug logger for the planner gateway. The whole AI failure path
 ...
 * produced no plan. `aiLog.debug` is silent unless `KIMERA_AI_DEBUG=1` (or
...
const debugEnabled = env.KIMERA_AI_DEBUG === true || env.LOG_LEVEL?.toLowerCase() === "debug";
