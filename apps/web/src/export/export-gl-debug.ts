 ...
 *
 * Opt-in (`?exportGlDebug=1` or `localStorage["orreris.exportGlDebug"] = "1"`), OFF by default so the normal
...
function debugEnabled(): boolean {
  ...
  try {
    if (typeof window !== "undefined") {
      ...
      else if (window.localStorage?.getItem("orreris.exportGlDebug") === "1") enabled = true;
    }
  } catch {
