 ...
 *
 * Kill switch (repo convention): `?videoPool=0` → localStorage `lumio.videoPool` → `VITE_VIDEO_POOL`
...
/** Resolution order: `?videoPool=0|1` → localStorage `lumio.videoPool` → `VITE_VIDEO_POOL` → true. */
export function getVideoPoolEnabled(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("lumio.videoPool");
