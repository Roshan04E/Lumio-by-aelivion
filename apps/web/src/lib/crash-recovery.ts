import type { ProjectGraph } from "@lumio-by-aelivion/shared";
...
/** Resolution order: `?crashRecovery=0|1` → localStorage `lumio.crashRecovery` → `VITE_CRASH_RECOVERY` → true. */
export function getCrashRecoveryEnabled(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("lumio.crashRecovery");
