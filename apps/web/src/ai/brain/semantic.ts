/**
 * Orreris Brain — tier-2 Semantic layer (B3). Catches PARAPHRASES of commands the lower tiers
...
async function loadRealEmbedder(): Promise<SemanticEmbedder | null> {
  ...
  if (lastError) {
    ...
    console.warn("Orreris Brain: local embedding model unavailable; tier-2 stays on exact matching.", lastError);
  }
  return null;
}
...
const PHRASES_KEY = "orreris.brain.phrases.v1";
...
const PLAN_CACHE_KEY = "orreris.brain.plancache.v2";
