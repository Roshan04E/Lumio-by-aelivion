...
const VOICE_STORE_KEY = "orreris.voice.tts.voice.v1";
...
function failNaturalVoice(reason: string): void {
  console.warn("[orreris] natural voice: failed to load Kokoro —", reason);
  ...
  try {
    ...
    worker.onmessage = (event: MessageEvent<import("./tts.worker").TtsWorkerResponse>) => {
      ...
      if (message.type === "progress") {
        ...
        console.debug(`[orreris] natural voice: ready on ${message.device === "webgpu" ? "GPU (WebGPU)" : "CPU (WASM)"}`);
        ...
        if (message.type === "chunk") {
          ...
          speakSessions.delete(message.id);
          console.warn("[orreris] natural voice: generation failed in worker —", message.message);
    ...
    worker.onerror = (event) => {
      ...
      if (!kokoroReady) {
        ...
        console.warn("[orreris] natural voice: worker error (engine kept alive)", event.message);
      }
    };
...
function playWav(wav: ArrayBuffer, token: number): Promise<void> {
  ...
  return new Promise<void>((resolve) => {
    ...
    element.play().catch((playError: unknown) => {
      ...
      console.warn("[orreris] natural voice: playback blocked", playError);
      settle();
    });
  });
}
...
export function startSpeechStream(): SpeechStreamHandle {
  ...
  const fallBackToSystem = (reason: string, strike: boolean) => {
    ...
    if (strike) {
      kokoroSlowStrikes += 1;
      console.warn(`[orreris] natural voice: ${reason} (strike ${kokoroSlowStrikes}) — using the system voice`);
      ...
      console.warn(`[orreris] natural voice: ${reason} — using the system voice`);
    }
    dropKokoroSession();
    engine = "system";
    if (pushedText) {
      ...
      while (token === speakToken && engine === "kokoro") {
        ...
        if (wav) {
          if (!firstChunkSeen) {
            firstChunkSeen = true;
            kokoroSlowStrikes = 0;
            console.debug(`[orreris] natural voice: first chunk in ${Math.round(performance.now() - (firstPushAt || startedAt))}ms`);
          }
        ...
        if (ended && firstChunkSeen) {
          ...
          if (chunkCount === before && queue.length === 0 && !workerDone && token === speakToken) {
            console.warn("[orreris] natural voice: stream stalled mid-reply — stopping this reply");
            dropKokoroSession();
            break;
          }
          continue;
