 ...
 *
 * Flag (repo convention): `?wcDecode=0|1` → localStorage `lumio.wcDecode` → `VITE_WC_DECODE` →
...
/** Resolution order: `?wcDecode=0|1` → localStorage `lumio.wcDecode` → `VITE_WC_DECODE` → TRUE. */
export function getWcPreviewDecodeEnabled(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("lumio.wcDecode");
