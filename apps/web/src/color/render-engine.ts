 ...
 *   2. `localStorage["orreris.colorEngine"]`    — dev opt-in / opt-out.
...
import { REGION_PASS_MODEL_DEFAULT } from "@orreris/shared";
...
export function getColorEngine(): ColorEngine {
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = normalize(window.localStorage?.getItem("orreris.colorEngine"));
 ...
 * RENDERER_MODE). The formal pixel sweep (`pnpm --filter @orreris/worker render:compare:pixels`
...
export function getRendererMode(): RendererMode {
  if (typeof window !== "undefined") {
    try {
      ...
      if (param) return param;
      const stored = normalizeMode(window.localStorage?.getItem("orreris.rendererMode"));
      if (stored) return stored;
    } catch {
      /* SSR / restricted storage — fall through */
    }
 ...
 * default it can never blank a comp; `?compositor=dom` / `localStorage["orreris.compositor"]` /
...
export function getCompositorMode(): CompositorMode {
  if (typeof window !== "undefined") {
    try {
      ...
      if (param) return param;
      const stored = normalizeCompositor(window.localStorage?.getItem("orreris.compositor"));
      if (stored) return stored;
    } catch {
      /* SSR / restricted storage — fall through */
    }
 ...
 * Resolution order: `?exportSingleContext=0|1` → localStorage `orreris.exportSingleContext` → VITE env → true.
 */
export function getExportSingleContext(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("orreris.exportSingleContext");
 ...
 * to canvas2D).
 *
 * Resolution order: `?exportWorkerScene=0|1` → localStorage `orreris.exportWorkerScene` → VITE env → true.
 */
export function getExportWorkerScene(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("orreris.exportWorkerScene");
      if (stored != null) return truthy(stored);
    } catch {
      /* SSR / restricted storage — fall through */
    }
 ...
 *
 * Resolution order: `?regionPasses=0|1` → localStorage `orreris.regionPasses` → `VITE_REGION_PASSES` →
...
export function getRegionPassesEnabled(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("orreris.regionPasses");
      if (stored != null) return truthy(stored);
    } catch {
      /* SSR / restricted storage — fall through */
    }
 ...
 *
 * Resolution order: `?proxyViewerCapture=0|1` → localStorage `orreris.proxyViewerCapture` →
...
export function getProxyViewerCaptureEnabled(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("orreris.proxyViewerCapture");
      if (stored != null) return truthy(stored);
    } catch {
      /* SSR / restricted storage — fall through */
    }
 ...
 *
 * Resolution order: `?singleCtxPreview=0|1` → localStorage `orreris.singleCtxPreview` →
...
export function getSingleCtxPreviewEnabled(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("orreris.singleCtxPreview");
      if (stored != null) return truthy(stored);
    } catch {
      /* SSR / restricted storage — fall through */
    }
 ...
 * (`pnpm --filter @orreris/worker governor:stress`, real Chrome): enforced run bounded peak at 7
...
export function getGlGovernorEnabled(): boolean {
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const stored = window.localStorage?.getItem("orreris.glGovernor");
      if (stored != null) return truthy(stored);
    } catch {
      /* SSR / restricted storage — fall through */
    }
