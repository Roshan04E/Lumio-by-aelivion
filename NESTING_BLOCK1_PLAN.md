...
1. `pnpm -r typecheck`.
2. `pnpm --filter @orreris/web editor:test` — ADD checks: (a) group folding produces one group draw
...
4. `pnpm --filter @orreris/worker render:compare:pixels` with a nested fixture (media child +
...
- 6c/6d (scene:compare / render:compare:pixels nested fixtures): **NOT added.** Authoring a new pixel fixture
  ...
  **What WAS run**: `PIXEL_BROWSER_CHANNEL=chrome pnpm --filter @orreris/worker scene:compare`
...
- `pnpm --filter @orreris/web editor:test`: all 148 checks pass (138 pre-existing + 10 new).
