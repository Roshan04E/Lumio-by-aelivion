 ...
 * Writes clips, titles, and transitions so a Lumio timeline can round-trip through Premiere/Resolve/Final
...
function reportUnsupportedLayerFeatures(layer: TimelineLayer, unsupported: ExternalExportReportItem[]): void {
  ...
  if (layer.effects.length > 0) {
    unsupported.push({
      ...
      message: `${layer.effects.length} Lumio effect(s) on "${layer.name}" have no FCPXML equivalent and are lossy on export.`,
      layerId: layer.id
    });
  }
