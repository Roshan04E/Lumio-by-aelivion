...
const FORBIDDEN_FRAGMENT_GLSL_PATTERNS: Array<{ pattern: RegExp; message: string }> = [
  { pattern: /#\s*version\b/, message: "Do not include #version; Orreris injects the shader harness." },
  { pattern: /\bvoid\s+main\s*\(/, message: "Do not define main(); provide only vec4 effect(vec2 uv)." },
  { pattern: /\bprecision\s+(lowp|mediump|highp)\b/, message: "Do not declare precision; Orreris injects it." },
  ...
  { pattern: /#\s*extension\b/, message: "Do not declare #extension; Orreris injects the shader harness." }
];
...
function effectTypeForManifest(
  ...
  warnings.push("Native effect manifests should declare effect.type with a supported Orreris effect id.");
  return COLOR_PIPELINE_EFFECT_TYPE;
}
