...
export async function readFontIdentity(bytes: ArrayBuffer): Promise<FontIdentity> {
  ...
  if (isWoff2(new Uint8Array(bytes))) {
    /**
     * Still refused HERE, deliberately, even though S3's `decompressFontIfNeeded` can now handle it.
     * This function's contract is "read what a font says about itself", and silently converting
     ...
     *
     * The decompressor lives in `@orreris/storage`, NOT here, and that is not organisational
     * tidiness: it is a ~300KB emscripten build that takes a Node branch (`require("fs")`) and it
     * cannot run in a browser at all. Sitting in `shared` it was pulled into every bundle that
     * imports this module — Vite's and Remotion's webpack alike, where it broke the render bundle
     * outright with "Can't resolve 'path'". Server-side is where it works and where it belongs.
     */
