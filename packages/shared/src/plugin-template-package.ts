...
export const orrerisTimelineTemplatePackageFormat = "orreris.timeline-template.package" as const;
export const orrerisTimelineTemplatePackageVersion = 1 as const;

export interface OrrerisTemplateAssetRef {
  id: string;
  fileName: string;
  fileType: string;
...
export interface OrrerisTimelineTemplatePackage {
  format: typeof orrerisTimelineTemplatePackageFormat;
  formatVersion: typeof orrerisTimelineTemplatePackageVersion;
  exportedAt: string;
  app: {
    name: "Orreris";
    company: "Pesamee Studio";
  };
  manifest: PluginTimelineTemplateManifest;
  graph: ProjectGraph;
  assets: OrrerisTemplateAssetRef[];
  preview?: {
...
export interface ApplyTimelineTemplatePackageInput {
  package: OrrerisTimelineTemplatePackage;
  projectId: string;
  projectTitle: string;
...
const timelineTemplatePackageSchema = z.object({
  format: z.literal(orrerisTimelineTemplatePackageFormat),
  formatVersion: z.literal(orrerisTimelineTemplatePackageVersion),
  exportedAt: z.string().min(1),
  app: z.object({
    name: z.literal("Orreris"),
    company: z.literal("Pesamee Studio")
  }),
  manifest: pluginTimelineTemplateManifestSchema,
  graph: z.custom<ProjectGraph>((value) => isProjectGraphLike(value), "Package graph is not a Orreris project graph."),
  assets: z
    .array(
      z.object({
        id: z.string().min(1),
...
export function buildTimelineTemplatePackage(input: BuildTimelineTemplatePackageInput): OrrerisTimelineTemplatePackage {
  const templateGraph = {
  ...
  const manifest = pluginTimelineTemplateManifestSchema.parse({
    ...
    name: title,
    version: "1.0.0",
    description: input.description?.trim() || `Orreris timeline template exported from ${title}.`,
    author: { name: "Pesamee Studio" },
    license: { type: "unknown" },
    tags: ["timeline", "template", "orreris"],
  ...
  return {
    format: orrerisTimelineTemplatePackageFormat,
    formatVersion: orrerisTimelineTemplatePackageVersion,
...
export function parseTimelineTemplatePackage(value: unknown): OrrerisTimelineTemplatePackage {
...
export function timelineTemplatePackageToJson(pkg: OrrerisTimelineTemplatePackage): string {
...
function collectAssetRefs(graph: ProjectGraph, assets: SourceAsset[]): OrrerisTemplateAssetRef[] {
...
function missingAssetWarnings(graph: ProjectGraph, availableAssetIds: string[], packageAssets: OrrerisTemplateAssetRef[]): string[] {
