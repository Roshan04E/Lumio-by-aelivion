...
- **Never touch the renderer.** No changes to `apps/web/src/components/VideoPreview.tsx` or `apps/worker/src/remotion/Root.tsx`. Verify with `pnpm --filter @lumio-by-aelivion/worker render:compare:pixels` (must stay ~0.245%).
...
- TS strict; `pnpm -r typecheck` IS the lint. Tests: `pnpm --filter @lumio-by-aelivion/shared actions:test`, `pnpm --filter @lumio-by-aelivion/web editor:test`.
