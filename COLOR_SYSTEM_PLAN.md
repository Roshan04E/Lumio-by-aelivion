# Kimera — Professional Color System (Phase 3 "Color" maturity)
...
> the Kimera-AI feature whose golden rule forbids it. The AI planner gains access to the new
...
Kimera is **professional-grade** (Premiere/Resolve target). The rule for color & effects: always
...
- `pnpm --filter @kimera-by-aelivion/worker render:compare:pixels` stays aligned (re-baselined at 13C.0).
- `pnpm --filter @kimera-by-aelivion/shared color:test` — CPU reference correctness (identity = passthrough;
  exposure brightens; saturation 0 = grayscale at correct luma weights; etc.).
- `pnpm --filter @kimera-by-aelivion/worker render:manifest <file>` spot-checks on real footage.
...
- **13C.0 — ✅ done.** Shared color engine (`color/` — compile + SVG emitter + CPU reference +
  ...
  CSS filters into `getCompositionColorFilter()` (SVG `url(#kimera-color-…)`); both renderers inject
...
- **13C.3b — ✅ built (unified WebGL media pipeline), flag-gated `rendererMode=webgl`, default still `legacy`
  ...
  - **Pixel harness extended:** `render-comparison-fixture.ts` now exposes keyed variants (plain-image,
    ...
    `pnpm --filter @kimera-by-aelivion/worker render:compare:pixels` (sweeps every fixture, rendererMode=webgl) —
