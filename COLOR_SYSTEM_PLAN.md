...
- `pnpm --filter @lumio-by-aelivion/worker render:compare:pixels` stays aligned (re-baselined at 13C.0).
- `pnpm --filter @lumio-by-aelivion/shared color:test` — CPU reference correctness (identity = passthrough;
  exposure brightens; saturation 0 = grayscale at correct luma weights; etc.).
- `pnpm --filter @lumio-by-aelivion/worker render:manifest <file>` spot-checks on real footage.
...
- **13C.3b — ✅ built (unified WebGL media pipeline), flag-gated `rendererMode=webgl`, default still `legacy`
  ...
  - **Pixel harness extended:** `render-comparison-fixture.ts` now exposes keyed variants (plain-image,
    ...
    `pnpm --filter @lumio-by-aelivion/worker render:compare:pixels` (sweeps every fixture, rendererMode=webgl) —
