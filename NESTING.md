...
Orreris already imports Premiere projects and **preserves nested sequences** (`external-timeline-adapter.ts` emits
...
1. **A nest is a real sequence** used as a clip. It lives at project level; the clip is a *reference*.
   Orreris: nested sequence = `TimelineComposition` in `ProjectGraph.compositions`; `graph.composition` stays the
...
4. **Duration is not live**: shortening the source sequence does NOT shrink placed clips — the overhang renders
   as empty (Premiere: black/silence; Orreris: transparent + silence, see §Alpha). Lengthening the source doesn't
